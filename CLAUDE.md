...
**Always use `bun` as the package manager** (the project uses `bun.lockb`, not `package-lock.json`):
...
**Pre-commit hooks** automatically run `bun run lint:fix && bun run format`. If hooks fail, commit with `--no-verify` at your own risk.
...
- **TypeScript** with strict mode enabled (including `exactOptionalPropertyTypes`)
...
1. **GitHub OAuth** (`src/lib/github-oauth.ts`):
   ...
   - `GET /api/auth/github/callback` → exchanges code for access token, creates JWT
...
**Suspense Boundaries**: Pages using `useSearchParams()` must be wrapped in `<Suspense>`. See `src/app/page.tsx` - it imports `LoginPage` from `src/app/login-page.tsx` to avoid static generation issues.

**Type Safety with `exactOptionalPropertyTypes`**: This compiler option is enabled. When working with optional properties:
```typescript
...
### Known Issues
- **Husky v10 deprecation**: The `.husky/pre-commit` file shows a deprecation warning. This is cosmetic and doesn't affect functionality.
- **Test users**: Email login uses hardcoded test users (`admin@example.com/admin123`, `user@example.com/user123`) - not production-ready.
